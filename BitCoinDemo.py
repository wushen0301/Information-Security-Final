...
class BlockchainSimulator:
    ...
    def mine_process(self):
        ...
        while True:
            #串接字串 (PrevHash + Transactions + Nonce)
            ...
            if new_hash.startswith(prefix):
                ...
                self.log("-" * 50)
                self.log(f"成功找到符合條件的 Nonce!")
                self.log(f"最終 Nonce: {nonce}")
                self.log(f"最終 Hash: {new_hash}")
                self.log(f"運算次數: {nonce + 1} 次")
                self.log(f"耗費時間: {elapsed_time:.4f} 秒")
                self.log("-" * 50)
                break
            
            #還沒找到符合的nonce，每十萬次顯示一次目前嘗試次數
            if nonce % 100000 == 0 and nonce > 0:
                self.log(f" -已嘗試 {nonce} 次，目前 Hash: {new_hash[:30]}")
